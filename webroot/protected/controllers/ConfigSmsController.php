...
class ConfigSmsController extends Controller
{
	...
	{
		$model=ConfigSms::model()->find();
		$model->scenario='update';
		// 如果需要AJAX验证反注释下面一行
		// $this->performAjaxValidation($model);

		if(isset($_POST['ConfigSms']))
		{
			$model->attributes=$_POST['ConfigSms'];
	...
	{
		$model=ConfigSms::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='config-sms-form')
		{
