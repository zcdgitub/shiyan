...
class AwardWeekSumController extends Controller
{
	/**
	...
	public function actionIndex($curSumType=1)
	{
		...
		$model=new AwardWeekSum('search');
		$model->unsetAttributes();  // clear any default values
		$model->awardWeekSumMemberinfo=new Memberinfo('search');
		$model->awardWeekSumType=new SumType('search');
		$model->awardWeekSumType->sum_type_id=$curSumType;

		if(isset($_GET['AwardWeekSum']))
		{
			$model->attributes=$_GET['AwardWeekSum'];
			if(isset($_GET['AwardWeekSum']['awardWeekSumMemberinfo']))
				$model->awardWeekSumMemberinfo->attributes=$_GET['AwardWeekSum']['awardWeekSumMemberinfo'];

		}
		if(!user()->isAdmin())
			$model->award_week_sum_memberinfo_id=user()->id;
		//获取奖金名称列表及生成grid列
		...
		$sumTypes=SumType::model()->findAll(['with'=>['sumConfigs'],'condition'=>"sum_config_date='week'",'order'=>'t.sum_type_id asc']);
	...
	{
		$model=new AwardWeekSumAll('search');
		$model->unsetAttributes();  // clear any default values
		$model->awardWeekSumMemberinfo=new Memberinfo('search');
		$model->award_week_sum_is_withdrawals=$withdrawals;
		if($withdrawals==0)
			$model->award_week_sum_date="<" . date('Y-m-d', time()-86400*date('w')+(date('w')>0?86400:-6*86400));
		if(!user()->isAdmin())
			$model->award_week_sum_memberinfo_id=user()->id;
		if(isset($_GET['AwardWeekSumAll']))
		{
			$model->attributes=$_GET['AwardWeekSumAll'];
			if(isset($_GET['AwardWeekSumAll']['awardWeekSumMemberinfo']))
				$model->awardWeekSumMemberinfo->attributes=$_GET['AwardWeekSumAll']['awardWeekSumMemberinfo'];

		}
		if(isset($_REQUEST['allWithdrawals']) && $_REQUEST['allWithdrawals']==1 && $model->award_week_sum_is_withdrawals==0)
		{
	...
	public function actionSalaryDel($id)
	{
		...
		$model->award_week_sum_is_withdrawals=1;
		$model->saveAttributes(['award_week_sum_is_withdrawals']);
		$this->log['target']=$model->awardWeekSumMemberinfo->memberinfo_account;
		$this->log['status']=LogFilter::SUCCESS;
		$this->log();
		user()->setFlash('success','会员' . $model->awardWeekSumMemberinfo->memberinfo_account . t('epmms',"工资单已删除并移入已发放"));
	...
	public function actionPayoff($member_id)
	{
		...
		$award=AwardWeekSumAll::model()->findByAttributes(['award_week_sum_memberinfo_id'=>$member_id]);
		$model->withdrawals_currency=$award->award_week_sum_currency;
		...
		{
			$award->award_week_sum_is_withdrawals=1;
			$award->saveAttributes(['award_week_sum_is_withdrawals']);
			$transaction->commit();
			$this->log['status']=LogFilter::SUCCESS;
			$this->log();
	...
	{
		$model=AwardWeekSum::model()->findByPk($id);
		if($model===null)
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='award-week-sum-form')
		{
