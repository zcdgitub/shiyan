...
class AwardMonthSumController extends Controller
{
	/**
	...
	public function actionIndex($curSumType=1)
	{
		...
		$model=new AwardMonthSum('search');
		$model->unsetAttributes();  // clear any default values
		$model->awardMonthSumMemberinfo=new Memberinfo('search');
		$model->awardMonthSumType=new SumType('search');
		$model->awardMonthSumType->sum_type_id=$curSumType;

		if(isset($_GET['AwardMonthSum']))
		{
			$model->attributes=$_GET['AwardMonthSum'];
			if(isset($_GET['AwardMonthSum']['awardMonthSumMemberinfo']))
				$model->awardMonthSumMemberinfo->attributes=$_GET['AwardMonthSum']['awardMonthSumMemberinfo'];

		}
		if(!user()->isAdmin())
			$model->award_month_sum_memberinfo_id=user()->id;
		//获取奖金名称列表及生成grid列
		...
		{
			//构造grid列
			$gridColumn[]=['name'=>'_award_' . $award->award_type_id,'header'=>t('epmms',$award->award_type_name),'filter'=>false];
		}
		$sumTypes=SumType::model()->findAll(['with'=>['sumConfigs'],'condition'=>"sum_config_date='month'",'order'=>'t.sum_type_id asc']);
	...
	{
		$model=new AwardMonthSumAll('search');
		$model->unsetAttributes();  // clear any default values
		$model->awardMonthSumMemberinfo=new Memberinfo('search');
		$model->award_month_sum_is_withdrawals=$withdrawals;
		if($withdrawals==0)
			$model->award_month_sum_date="<" . date('Y-m-01');
		if(!user()->isAdmin())
			$model->award_month_sum_memberinfo_id=user()->id;
		if(isset($_GET['AwardMonthSumAll']))
		{
			$model->attributes=$_GET['AwardMonthSumAll'];
			if(isset($_GET['AwardMonthSumAll']['awardMonthSumMemberinfo']))
				$model->awardMonthSumMemberinfo->attributes=$_GET['AwardMonthSumAll']['awardMonthSumMemberinfo'];
		}
		if(isset($_REQUEST['allWithdrawals']) && $_REQUEST['allWithdrawals']==1 && $model->award_month_sum_is_withdrawals==0)
		{
	...
	public function actionSalaryDel($id)
	{
		...
		$model->award_month_sum_is_withdrawals=1;
		$model->saveAttributes(['award_month_sum_is_withdrawals']);
		$this->log['target']=$model->awardMonthSumMemberinfo->memberinfo_account;
		$this->log['status']=LogFilter::SUCCESS;
		$this->log();
		user()->setFlash('success','会员' . $model->awardMonthSumMemberinfo->memberinfo_account . t('epmms',"工资单已删除并移入已提现"));
	...
	public function actionPayoff($member_id)
	{
		...
		$award=AwardMonthSumAll::model()->findByAttributes(['award_month_sum_memberinfo_id'=>$member_id]);
		$model->withdrawals_currency=$award->award_month_sum_currency;
		...
		{
			$award->award_month_sum_is_withdrawals=1;
			$award->saveAttributes(['award_month_sum_is_withdrawals']);
			$transaction->commit();
			$this->log['status']=LogFilter::SUCCESS;
			$this->log();
	...
	{
		$model=AwardMonthSum::model()->findByPk($id);
		if($model===null)
	...
	{
		$model=AwardMonthSumAll::model()->findByPk($id);
		if($model===null)
			throw new CHttpException(404,'The requested page does not exist.');
		return $model;
	}
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='award-month-sum-form')
		{
