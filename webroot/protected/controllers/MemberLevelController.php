...
class MemberLevelController extends Controller
{
	...
	public function actionCreate()
	{
		...
		{
			/*echo "<Pre>";
			var_dump($_POST['MemberLevel']);
			die;*/
			$model->attributes=$_POST['MemberLevel'];
			$this->log['target']=$model->member_level_level;
			if($model->save(true,array('member_level_name')))
			{
				$this->log['status']=LogFilter::SUCCESS;
				$this->log();
				user()->setFlash('success',"{$this->actionName}“{$model->showName}”" . t('epmms',"成功"));
				$this->redirect(array('view','id'=>$model->member_level_level));
			}
			else
			{
	...
	public function actionUpdate($id)
	{
		...
		if(isset($_POST['MemberLevel']))
		{
			$model->attributes=$_POST['MemberLevel'];
			$this->log['target']=$model->member_level_level;
			if($model->save(true,array('member_level_name','member_level_level')))
			{
				$this->log['status']=LogFilter::SUCCESS;
				$this->log();
				user()->setFlash('success',"{$this->actionName}“{$model->showName}”" . t('epmms',"成功"));
	...
	{
		if(isset($_POST['ajax']) && $_POST['ajax']==='member-level-form')
		{
