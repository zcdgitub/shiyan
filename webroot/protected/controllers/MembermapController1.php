...
class MembermapController extends Controller
{
    ...
    public function actionGetLevelCount()
    {
    	...
    	$levelCount=[];
    		...
    		$levelCount[-1][0]=(string)new DbEvaluate("get_order_under_product_count_date(:id,'1 month'::interval,:order)",[':id'=>user()->id,':order'=>1]);
            $levelCount[-1][1]=(string)new DbEvaluate("get_order_under_product_count_date(:id,'1 month'::interval,:order)",[':id'=>user()->id,':order'=>2]);
            $levelCount[0][0]=(string)new DbEvaluate("get_order_under_product_count_date(:id,(get_config_award(612) || ' days')::interval,:order)",[':id'=>user()->id,':order'=>1]);
            $levelCount[0][1]=(string)new DbEvaluate("get_order_under_product_count_date(:id,(get_config_award(612) || ' days')::interval,:order)",[':id'=>user()->id,':order'=>2]);
    	for($i=1;$i<=7;$i++)
    	{
