...
class ModelCode extends CCodeModel
{
	...
	public function rules()
	{
		return array_merge(parent::rules(), array(
			...
			array('modelName',  'ext.validators.Account','allowZh'=>true),
		));
	}
	...
	public function attributeLabels()
	{
		return array_merge(parent::attributeLabels(), array(
			...
			'modelClass'=>'Model类',
			'baseClass'=>'基类',
			'buildRelations'=>'生成Relations',
			'commentsAsLabels'=>'使用列注释作为Attribute Labels',
			'connectionId'=>'数据库连接',
			'modelName'=>'Model标题'
		));
	}
	...
	public function prepare()
	{
		...
		foreach($tables as $table)
		{
			...
			$this->nameColumn=$this->guessNameColumn($table->columns);
			$params=array(
	...
	public function generateRules($table)
	{
		...
		$unique=array();
		$exist=array();
		$onlyDate=array();//纯日期部分
		$password=array();//hash密码
		$unsafe=array();
		$clientIP=array();
		$allowNull=array();//转换empty为null
		$decimal=array();
		foreach($table->columns as $column)
		{
			...
			if($r)
				...
				$allowNull[]=$column->name;
			foreach($this->getColumnIndex($column->name) as $colIndex)
			{
				if($colIndex['isunique']=='true')
				{
					$unique[]=$column->name;
					break;
				}
			...
			{
				$relations=$this->getRelations($this->modelClass);
				foreach($relations as $relationName=>$relation)
				{
					$relation=eval('return ' . $relation . ';');
					if($relation[2]!=$column->name)
					{
						break;
					}
					if($relation[0]==Model::BELONGS_TO)
					{
						if($relation[1]==$this->modelClass)
						{
						...
						else
						{
							...
							$tableSchema=$foreignModel->tableSchema;
						}
						$exist[$relation[1]][$tableSchema->primaryKey][]=$column->name;
					}
				}
			...
			if(!strcasecmp($cname,'sex'))
				$sex[]=$column->name;
			else if(!strcasecmp($cname,'is_enable'))
				$enable[]=$column->name;
			else if(!strcasecmp($cname,'is_preset'))
				$preset[]=$column->name;
			else if(!strcasecmp($cname,'phone'))
				$phone[]=$column->name;
			else if(!strcasecmp($cname,'mobi'))
				$mobi[]=$column->name;
			else if(!strcasecmp($cname,'zipcode'))
				$zipcode[]=$column->name;
			else if(!strcasecmp($cname,'qq'))
				$qq[]=$column->name;
			else if(!strcasecmp($cname,'msn'))
				$msn[]=$column->name;
			else if(!strcasecmp($cname,'name'))
				$name[]=$column->name;
			else if(!strcasecmp($cname,'account'))
				$account[]=$column->name;
			else if(!strcasecmp($cname,'password') || !strcasecmp($cname,'password2') || !strcasecmp($cname,'password3') || !strcasecmp($cname,'answer'))
				$password[]=$column->name;
			else if(!strcasecmp($cname,'birthday'))
				$onlyDate[]=$column->name;
			else if(!strcasecmp($cname,'register_ip') || !strcasecmp($cname,'client_ip') || !strcasecmp($cname,'access_ip') || !strcasecmp($cname,'last_ip'))
				$clientIP[]=$column->name;
			else if(strncmp($column->dbType,'decimal',7)===0||strncmp($column->dbType,'numeric',7)===0)
				$decimal[$column->precision][$column->scale][]=$column->name;
		...
		if($allowNull!==array())
			$rules[]="array('".implode(', ',$allowNull)."', 'filter','filter'=>array(\$this,'empty2null'))";
		if($required!==array())
		...
		if($exist!==array())
		{
			foreach($exist as $className=>$attribute)
				foreach($attribute as $attributeName=>$cols);
			$rules[]="array('".implode(', ',$cols)."', 'exist', 'className'=>'{$className}','attributeName'=>'{$attributeName}')";
		}
		if($sex!==array())
			$rules[]="array('".implode(', ',$sex)."', 'ext.validators.Sex')";
		if($enable!==array())
			$rules[]="array('".implode(', ',$enable)."', 'ext.validators.Enable')";
		if($preset!==array())
			$rules[]="array('".implode(', ',$preset)."', 'ext.validators.Preset')";
		if($phone!==array())
			$rules[]="array('".implode(', ',$phone)."', 'ext.validators.Phone')";
		if($mobi!==array())
			$rules[]="array('".implode(', ',$mobi)."', 'ext.validators.Phone','allowTel'=>false)";
		if($zipcode!==array())
			$rules[]="array('".implode(', ',$zipcode)."', 'ext.validators.Zipcode')";
		if($qq!==array())
			$rules[]="array('".implode(', ',$qq)."', 'ext.validators.QQ')";
		if($msn!==array())
			$rules[]="array('".implode(', ',$msn)."', 'ext.validators.QQ','allowTel'=>false,'allowQQ'=>false)";
		if($name!==array())
			$rules[]="array('".implode(', ',$name)."', 'ext.validators.Account','allowZh'=>true)";
		if($account!==array())
			$rules[]="array('".implode(', ',$account)."', 'ext.validators.Account','allowZh'=>false)";
		if($password!==array())
			$rules[]="array('".implode(', ',$password)."', 'ext.validators.Password','on'=>'update')";
		if($onlyDate!==array())
			$rules[]="array('".implode(', ',$onlyDate)."', 'ext.validators.Date')";
		if($clientIP!==array())
			$rules[]="array('".implode(', ',$clientIP)."', 'default','value'=>webapp()->request->userHostAddress)";
		if($decimal!==array())
		{
			foreach($decimal as $precision=>$dec)
				foreach($dec as $scale=>$cols)
					$rules[]="array('".implode(', ',$cols)."', 'ext.validators.Decimal','precision'=>$precision,'scale'=>$scale)";
		}
		if($safe!==array())
			$rules[]="array('".implode(', ',$safe)."', 'safe')";
		if($unsafe!==array())
			$rules[]="array('".implode(', ',$unsafe)."', 'unsafe')";
	...
	protected function generateRelations()
	{
		...
		foreach(Yii::app()->{$this->connectionId}->schema->getTables($schemaName) as $table)
		{
			...
			if ($this->isRelationTable($table))
			{
				...
				$relations[$className0][$relationName]="array(Model::MANY_MANY, '$className1', '$unprefixedTableName($pks[0], $pks[1])')";
				...
				$relations[$className1][$relationName]="array(Model::MANY_MANY, '$className0', '$unprefixedTableName($pks[1], $pks[0])')";
			}
			else
			{
				...
				foreach ($table->foreignKeys as $fkName => $fkEntry)
				{
					...
					$relations[$className][$relationName]="array(Model::BELONGS_TO, '$refClassName', '$fkName')";
					...
					while(isset($relations[$refClassName][$relationName]))
						$relationName=$rawName.($i++);
					$relations[$refClassName][$relationName]="array(Model::$relationType, '$className', '$fkName')";
				}
			}
		}
		return $relations;
	 ...
	 * @return string
	 */
	public static function getShortColumnName($name)
	{
	...
	{
		//mysql
		//$sql="show index from {$this->tableName} where Column_name=:columnName";

		//pgsql
		$sql="select * from ep_index where indrelid=:tableName::regclass and :columnName = any(indkey_names);";
		$command=Yii::app()->{$this->connectionId}->createCommand($sql);
		$command->bindParam(':tableName',$this->tableName);
		$command->bindParam(':columnName',$name);
