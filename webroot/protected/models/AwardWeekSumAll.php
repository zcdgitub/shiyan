...
/**
 * This is the model class for table "{{award_week_sum_all}}".
 *
 * The followings are the available columns in table '{{award_week_sum_all}}':
 * @property integer $award_week_sum_id
 * @property integer $award_week_sum_memberinfo_id
 * @property string $award_week_sum_currency
 * @property string $award_week_sum_date
 * @property string $award_week_sum_add_date
 * @property integer $award_week_sum_is_withdrawals
 *
 * The followings are the available model relations:
 * @property  */
class AwardWeekSumAll extends Model
{
	//模型标题
	public $modelName='周工资';
	//命名字段
	...
	public function rules()
	{
		...
		return array(
			array('award_week_sum_add_date, award_week_sum_is_withdrawals', 'filter','filter'=>array($this,'empty2null')),
			array('award_week_sum_memberinfo_id, award_week_sum_currency, award_week_sum_date', 'required'),
			array('award_week_sum_memberinfo_id, award_week_sum_is_withdrawals', 'numerical', 'integerOnly'=>true),
			array('award_week_sum_add_date', 'length', 'max'=>3),
			array('award_week_sum_memberinfo_id, award_week_sum_date', 'unique'),
			array('award_week_sum_memberinfo_id', 'exist', 'className'=>'Memberinfo','attributeName'=>'memberinfo_id'),
			array('award_week_sum_currency', 'ext.validators.Decimal','precision'=>16,'scale'=>2),
			// The following rule is used by search().
			// @todo Please remove those attributes that should not be searched.
			array('award_week_sum_id, award_week_sum_memberinfo_id, award_week_sum_currency, award_week_sum_date, award_week_sum_add_date, award_week_sum_is_withdrawals', 'safe', 'on'=>'search'),
		);
	}
	...
	public function payoff()
	{
		...
		$model->withdrawals_currency=$this->award_week_sum_currency;

		//$sumtype-id为null,为第1个财务类型
		$finance=Finance::getFinanceSumType($this->award_week_sum_memberinfo_id);
		...
		{
			$this->award_week_sum_is_withdrawals=1;
			$this->saveAttributes(['award_week_sum_is_withdrawals']);
			$transaction->commit();
			return true;
		}
		else
	...
	public function search()
	{
		...
		$sort=new Sort('AwardWeekSumAll');
		$sort->defaultOrder=array('award_week_sum_id'=>Sort::SORT_ASC);
		...
		$criteria->compare('award_week_sum_currency',$this->award_week_sum_currency,true);
		$criteria->compare('award_week_sum_date',$this->award_week_sum_date,true);
		$criteria->compare('award_week_sum_add_date',$this->award_week_sum_add_date,true);
		...
		$criteria->with=array('awardWeekSumMemberinfo');
	 ...
	 * @return AwardWeekSumAll the static model class
	 */
