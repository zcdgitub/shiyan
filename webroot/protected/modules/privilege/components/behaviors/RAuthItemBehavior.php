...
class RAuthItemBehavior extends CBehavior
{
	...
	* @return string the markup.
	*/
	public function getNameLink($page)
	{
	...
	*/
	public function getDeleteRoleLink()
	{
		...
		{
			return CHtml::imageButton(themeBaseUrl() . '/images/button/yello-mid-del2.gif', array(
				'submit'=>array('authItem/delete', 'name'=>urlencode($this->owner->name)),
				'confirm'=>t('epmms', '你确定要删除这个角色吗？'),
				'class'=>'delete-link',
				'csrf'=>Yii::app()->request->enableCsrfValidation,
			));
		}
	}
	public function getUpdateRoleLink()
	{
		return CHtml::imageButton(themeBaseUrl() . '/images/button/yello-mid-edit2.gif', array(
			'submit'=>array('authItem/update', 'name'=>urlencode($this->owner->name)),
			'csrf'=>Yii::app()->request->enableCsrfValidation
		));
	}
	/**
	...
	* @return string the markup.
	*/
	public function getRevokePermissionLink(CAuthItem $role,$page)
	{
		$csrf = Rights::getDataCsrf();
		
		return CHtml::checkBox(t('epmms', '撤消'),true, array(
			'onclick'=>"
				jQuery.ajax({
					type:'POST',
					...
					data:{ ajax:1 $csrf  },
					success:function() {
						$('#permissions').load('".Yii::app()->controller->createUrl('authItem/permissions',['page'=>$page])."', { ajax:1 $csrf });
					}
				});
	...
	* @return string the markup.
	*/
	public function getAssignPermissionLink(CAuthItem $role,$page)
	{
		$csrf = Rights::getDataCsrf();
		
		return CHtml::checkBox(t('epmms', '授权'), false, array(
			'onclick'=>"
				jQuery.ajax({
					type:'POST',
