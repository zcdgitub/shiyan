...
//            ]);
            $data = ActivationRecord::model()->findAll([
                'select'=>'activation_member_id',
                'condition' => "activation_add_time >='".date('Y-m-d H:i:s',$startTime)."' and activation_add_time <='".date('Y-m-d H:i:s',$endTime)."'",
...
//            Sms::model()->sendAward($mysystem->getPeriod());
     ...
     * @return bool
     */
    public function addJackpotRecord($uid,$money,$type,$startTime,$endTime,$number){
     ...
     * 添加尾单获奖记录并增加获奖者余额
     */
    public function addEndJackpot($uid,$money,$startTime,$endTime,$type,$group,$number){
        $transaction=webapp()->db->beginTransaction();
        try{
            // 增加奖池获奖者记录
            $ret = $this->addJackpotRecord($uid,$money,$type,$startTime,$endTime,$number);
            // 转账明细
     ...
     * 添加奖池信息记录
     */
    public function addJackpotInfo($model){
        try{
            $infoModel = new JackpotInfo();
            $infoModel->info_start_time = date('Y-m-d H:i:s',$model->config_jackpot_start_time);
            $infoModel->info_end_time   = date('Y-m-d H:i:s',$model->config_jackpot_end_time);
            $infoModel->info_start_balance = $model->config_jackpot_start_balance;
            $infoModel->info_lucky_balance = $model->config_jackpot_lucky_balance;
            $infoModel->info_end_balance   = $model->config_jackpot_end_balance;
            $infoModel->info_number         = $model->config_jackpot_number;
            $infoModel->save();
        }catch(Exception $e){
            throw $e;
            return EError::ERROR;
