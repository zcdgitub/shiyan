...
foreach($sumTypes as $sum)
{
	...
	{
		$tabParameters[$sum->sum_type_id]=['title'=>t('epmms',$sum->sum_type_name),'view'=>'_index','data'=>['model'=>$model,'gridColumn'=>$gridColumn,'curSumType'=>$sum->sum_type_id]];
	}
	else
	{
		$tabParameters[$sum->sum_type_id]=['title'=>t('epmms',$sum->sum_type_name),'url'=>$this->createUrl('awardYearSum/index',['curSumType'=>$sum->sum_type_id])];
	}
}
