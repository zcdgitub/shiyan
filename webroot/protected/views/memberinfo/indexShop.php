...
foreach($transferType as $isVerify=>$title)
{
	...
	{
		$tabParameters[$isVerify]=['title'=>$title,'view'=>'_indexShop','data'=>['model'=>$model,'isVerifyType'=>$isVerifyType,'isAgent'=>$isAgent]];
	}
	else
	{
		$tabParameters[$isVerify]=['title'=>$title,'url'=>$this->createUrl('memberinfo/indexShop',['isVerifyType'=>$isVerify,'isAgent'=>$isAgent])];
	}
}
