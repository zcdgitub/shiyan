 ...
 * @since 1.0
 */
class AbleTransferDstType extends CValidator
{
	...
	protected function validateAttribute($object,$attribute)
	{
		...
		$src_type=$object->transfer_src_finance_type;

		if($src_type==1 && $value==3)
		{
			$this->addError($object,$attribute,t('epmms','奖金币不能转为注册金'));
			return;
		}

        if($src_type!=$value){
        	$this->addError($object,$attribute,t('epmms','不同币种不能互转'));
			return;
        }
		$config=TransferConfig::model()->find();
		if(array_search($value,$config->transfer_config_dst_type)===false)
		{
