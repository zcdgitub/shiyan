 ...
 * @since 1.0
 */
class BuyCount extends CValidator
{
	/**
	...
	protected function validateAttribute($object,$attribute)
	{
		...
		$this->max=$this->max=Product::model()->find('product_id=:id',[':id'=>$object->orders_product_product_id])->product_stock;
		...
		{
			$message=$this->tooBig!==null?$this->tooBig:t('epmms','{attribute} 超过了库存.');
			$this->addError($object,$attribute,$message);
		}
	}
	...
	public function clientValidateAttribute($object,$attribute)
	{
		...
		$product=Product::model()->find('product_id=:id',[':id'=>$object->orders_product_product_id]);
		$this->max=$product->product_stock;
		$htmlOptions=[];
		$attribute="[{$object->orders_product_product_id}orders_product_currency";
		CHtml::resolveNameID($object, $attribute,$htmlOptions);
...
";
		...
		$js.="\$('#{$htmlOptions['id']}').text(value*{$product->product_price});var product_money=0;\$('.money').each(function(){product_money+=parseFloat($(this).text())});\$('#Orders_orders_currency_show').text(product_money + '￥');\$('#Orders_orders_currency').val(product_money);";
