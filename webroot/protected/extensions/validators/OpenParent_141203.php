 ...
 * @since 1.0
 */
class OpenParent_141203 extends CValidator
{
	...
	protected function validateAttribute($object,$attribute)
	{
		...
		if($this->isEmpty($mtype))
			return;
		$rid=$object->membermap_recommend_id;

		if($rid!=$value && $mtype==4 && $rid==1)
		{
			$this->addError($object,$attribute,'接点人需要与推荐人相同');
			return;
		}
		$rmember=Membermap::model()->findByPk($rid);
		if(is_null($rmember))
			return;
		if($rid!=$value && $mtype==4 && $rmember->membermap_membertype_level==4)
		{
			$this->addError($object,$attribute,'接点人需要与推荐人相同');
			return;
		}
