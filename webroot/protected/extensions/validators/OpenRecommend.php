 ...
 * @since 1.0
 */
class OpenRecommend extends CValidator
{
	...
	protected function validateAttribute($object,$attribute)
	{
		...
		if($this->isEmpty($value))
			return;
		if(!Membermap::model()->exists("membermap_is_verify=1 and membermap_id=:id and membermap_recommend_path like :rpath || '%' ",[':rpath'=>user()->map->membermap_recommend_path,'id'=>$value]))
		{
			$message=t('epmms','不是当前登录的会员或当前登录会员的下级');
