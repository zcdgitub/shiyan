 ...
 * @since 1.0
 */
class AblePercent2 extends CValidator
{
	...
	protected function validateAttribute($object,$attribute)
	{
		...
		$value1=$object->membermap_percent1;
		$message=t('epmms','未正确填写');
		if(is_null($value))
		{
		...
		if($value!=50)
		{
			$this->addError($object,$attribute,t('epmms','必须50%'));
			return;
		}
		if(!$this->isEmpty($value1))
		{
			if($value1+$value>100)
			{
				$this->addError($object,$attribute,t('epmms','合计不能超过100%'));
				return;
			}
			if($value1+$value<100)
			{
				$this->addError($object,$attribute,t('epmms','合计不能小于100%'));
				return;
			}
		}
	}
