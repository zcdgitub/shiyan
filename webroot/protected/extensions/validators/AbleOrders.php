 ...
 * @since 1.0
 */
class AbleOrders extends CValidator
{
	...
	protected function validateAttribute($object,$attribute)
	{
		...
		$message=t('epmms','未正确选择产品');
		if(is_null($value))
		{
			$this->addError($object,$attribute,$message);
			return;
		}
		if($this->isEmpty($_POST['Membermap']['membermap_membertype_level']))
		{
			$this->addError($object,$attribute,t('epmms','请先选择会员类型'));
			return;
		}
		$membertype=$_POST['Membermap']['membermap_membertype_level'];
		$membertype_model=MemberType::model()->findByPk($membertype);
		if(is_null($membertype_model))
		{
			$this->addError($object,$attribute,t('epmms','请先选择会员类型'));
			return;
		}
		if($value!=$membertype_model->membertype_money)
		{
			$this->addError($object,$attribute,t('epmms','订单金额与会员类型不符'));
			return;
		}
	}
}
