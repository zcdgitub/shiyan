 ...
 * 'allowedExtensions'=>array("jpg"),//array("jpg","jpeg","gif","exe","mov" and
 * etc...
...
class EAjaxUpload extends CWidget
{
    ...
    public function run()
    {
        ...
        Yii::app ()->clientScript->registerScriptFile ( $baseUrl . (YII_DEBUG ? '/fineuploader-3.0.js' : '/fineuploader-3.0.min.js'), CClientScript::POS_HEAD );

        $this->css = (! empty ( $this->css )) ? $this->css : $baseUrl . '/fineuploader.css';
        ...
        $this->config['request'] = array_merge ( $postParams, $this->config['request'] );

        $config = array(
            'element'=>new CJavaScriptExpression("document.getElementById('$this->id')"),
            'debug' => false,
            'multiple' => false
        );
        ...
        $jscode=<<<EOF
		var filelimituploader = new qq.FineUploader($config);
EOF;
        Yii::app ()->getClientScript ()->registerScript($this->id, $jscode, CClientScript::POS_LOAD );
    }
}