...
class AlipayNotify {
	...
	*$transport 访问模式
     */
    function __construct($partner, $key, $sign_type, $_input_charset = "GBK", $transport= "http") {
	...
	*返回的验证结果：true/false
    ...
    function notify_verify() {
        ...
        if($this->transport == "https") {
            $veryfy_url = $this->gateway. "service=notify_verify" ."&partner=" .$this->partner. "&notify_id=".$_REQUEST["notify_id"];
        } else {
            $veryfy_url = $this->gateway. "partner=".$this->partner."&notify_id=".$_REQUEST["notify_id"];
        }
        //LogFilter::log(['category'=>'operate','source'=>'在线充值','operate'=>'支付通知','user'=>'Guest','role'=>'Guest','target'=>'testPay3','status'=>LogFilter::SUCCESS]);
        $veryfy_result = $this->get_verify($veryfy_url);
        //LogFilter::log(['category'=>'operate','source'=>'在线充值','operate'=>'支付通知','user'=>'Guest','role'=>'Guest','target'=>'testPay4','status'=>LogFilter::SUCCESS]);
        //生成签名结果
		if(empty($_REQUEST)) {							//判断POST来的数组是否为空
			...
			//写日志记录
			//log_result("veryfy_result=".$veryfy_result."\n notify_url_log:sign=".$_REQUEST["sign"]."&mysign=".$this->mysign.",".create_linkstring($sort_post));
