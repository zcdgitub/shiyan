	...
	*/	
	class EExcelView extends GridView
	{
		...
		public function init()
		{
			...
			if($this->grid_mode!='grid') //if needed disable paging to export all data
		...
		public function renderRow($row)
		{
			...
			foreach($this->columns as $n=>$column)
			{
				...
				{
					$value=$this->evaluateExpression($column->value ,array('data'=>$this->dataProvider->data[$row],'row'=>$row));
				}
				elseif($column->name!==null && $column instanceof DataColumn)
				{
					$value= $column->value($data[$row], $column->name);
				...
				$a++;
				$cell = $this->objPHPExcel->getActiveSheet()->setCellValueExplicit($this->columnName($a).($row+2) , strip_tags(is_object($value)?CHtml::resolveValue($value,'showName'):$value),PHPExcel_Cell_DataType::TYPE_STRING, true);
