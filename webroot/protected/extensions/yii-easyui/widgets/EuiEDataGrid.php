...
class EuiEDataGrid extends EuiControl
{
	...
	public function init()
	{
		...
		$cs->registerScriptFile($scriptUrl.'/plugins/jquery.edatagrid.js');
		$this->initToolbar();
	...
	{									 				
		...
		if (!empty($this->saveUrl))
			$config['saveUrl'] = $this->saveUrl;
		if (!empty($this->updateUrl))
			$config['updateUrl'] = $this->updateUrl;
		if (!empty($this->destroyUrl))
			$config['destroyUrl'] = $this->destroyUrl;
		...
		if (!empty($config))
		{
			$script = EuiJavaScript::encodeId($this->id).".edatagrid(\n";
